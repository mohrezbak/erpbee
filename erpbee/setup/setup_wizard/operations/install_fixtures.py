...
from erpbee.accounts.doctype.account.account import RootNotEditable
from erpbee.regional.address_template.setup import set_up_address_templates
...
def install(country=None):
	...
	from erpbee.setup.setup_wizard.data.industry_type import get_industry_types
	...
	base_path = frappe.get_app_path("erpbee", "hr", "doctype")
	...
	base_path = frappe.get_app_path("erpbee", "stock", "doctype")
	...
	from erpbee.buying.doctype.supplier_scorecard.supplier_scorecard import make_default_records
	...
	# path = frappe.get_app_path('erpbee', 'regional', frappe.scrub(country))
	# if os.path.exists(path.encode("utf-8")):
	# 	frappe.get_attr("erpbee.regional.{0}.setup.setup_company_independent_fixtures".format(frappe.scrub(country)))()
...
def add_uom_data():
	# add UOMs
	uoms = json.loads(open(frappe.get_app_path("erpbee", "setup", "setup_wizard", "data", "uom_data.json")).read())
	for d in uoms:
	...
	uom_conversions = json.loads(open(frappe.get_app_path("erpbee", "setup", "setup_wizard", "data", "uom_conversion_data.json")).read())
