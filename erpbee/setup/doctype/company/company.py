...
import functools
from erpbee.accounts.doctype.account.account import get_account_currency
...
class Company(NestedSet):
	...
	def create_default_tax_template(self):
		from erpbee.setup.setup_wizard.operations.taxes_setup import create_sales_tax
		create_sales_tax({
	...
	def on_update(self):
		...
		if not frappe.db.get_value("Department", {"company": self.name}):
			from erpbee.setup.setup_wizard.operations.install_fixtures import install_post_company_fixtures
...
def enqueue_replace_abbr(company, old, new):
	...
	frappe.enqueue('erpbee.setup.doctype.company.company.replace_abbr', **kwargs)
...
def install_country_fixtures(company):
	...
	path = frappe.get_app_path('erpbee', 'regional', frappe.scrub(company_doc.country))
	if os.path.exists(path.encode("utf-8")):
		try:
			module_name = "erpbee.regional.{0}.setup.setup".format(frappe.scrub(company_doc.country))
			frappe.get_attr(module_name)(company_doc, False)
		except Exception as e:
			frappe.log_error()
			frappe.throw(_("Failed to setup defaults for country {0}. Please contact support@erpbee.com").format(frappe.bold(company_doc.country)))
