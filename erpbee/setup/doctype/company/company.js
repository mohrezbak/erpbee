...
frappe.ui.form.on("Company", {
	...
	setup: function(frm) {
		erpbee.company.setup_queries(frm);
	...
	refresh: function(frm) {
		...
		erpbee.company.set_chart_of_accounts_options(frm.doc);
	...
	delete_company_transactions: function(frm) {
		frappe.verify_password(function() {
			...
			function(data) {
				...
				frappe.call({
					method: "erpbee.setup.doctype.company.delete_company_transactions.delete_company_transactions",
					args: {
...
erpbee.company.set_chart_of_accounts_options = function(doc) {
	var selected_value = doc.chart_of_accounts;
	if(doc.country) {
		return frappe.call({
			method: "erpbee.accounts.doctype.account.chart_of_accounts.chart_of_accounts.get_charts_for_country",
			args: {
				"country": doc.country,
				"with_standard": true
			},
...
cur_frm.cscript.change_abbr = function() {
	...
	dialog.fields_dict.update.$input.click(function() {
		...
		return frappe.call({
			method: "erpbee.setup.doctype.company.company.enqueue_replace_abbr",
			args: {
...
erpbee.company.setup_queries = function(frm) {
	$.each([
		...
		erpbee.company.set_custom_query(frm, v);
	});
...
erpbee.company.set_custom_query = function(frm, v) {
	var filters = {
