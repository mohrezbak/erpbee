...
import frappe
from erpbee.accounts.doctype.cash_flow_mapper.default_cash_flow_mapper import DEFAULT_MAPPERS
from .default_success_action import get_default_success_action
from frappe import _
from frappe.utils import cint
from frappe.desk.page.setup_wizard.setup_wizard import add_all_roles_to
from frappe.custom.doctype.custom_field.custom_field import create_custom_field
from erpbee.setup.default_energy_point_rules import get_default_energy_point_rules

default_mail_footer = """<div style="padding: 7px; text-align: right; color: #888"><small>Sent via
	<a style="color: #888" href="http://erpbee.org">ERPBee</a></div>"""
...
def check_setup_wizard_not_completed():
	if cint(frappe.db.get_single_value('System Settings', 'setup_complete') or 0):
		message = """ERPBee can only be installed on a fresh site where the setup wizard is not completed.
...
def add_standard_navbar_items():
	...
	erpbee_navbar_items = [
		{
			...
			'item_type': 'Route',
			'route': 'https://erpbee.com/docs/user/manual',
			'is_standard': 1
		},
		{
			'item_label': 'User Forum',
			'item_type': 'Route',
			'route': 'https://discuss.erpbee.com',
			'is_standard': 1
		},
		{
			'item_label': 'Report an Issue',
			'item_type': 'Route',
			'route': 'https://github.com/frappe/erpbee/issues',
			'is_standard': 1
		}
	]
	...
	for item in erpbee_navbar_items:
