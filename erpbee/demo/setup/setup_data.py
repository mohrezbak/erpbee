...
from erpbee.demo.domains import data
...
def setup_holiday_list():
	...
	frappe.set_value("Company", erpbee.get_default_company(), "default_holiday_list", holiday_list.name)


def setup_user():
	frappe.db.sql('delete from tabUser where name not in ("Guest", "Administrator")')
	for u in json.loads(open(frappe.get_app_path('erpbee', 'demo', 'data', 'user.json')).read()):
...
def setup_user_roles(domain):
	user = frappe.get_doc('User', 'demo@erpbee.com')
...
def setup_account():
	...
	data = json.loads(open(frappe.get_app_path('erpbee', 'demo', 'data',
		'account.json')).read())
	for d in data:
