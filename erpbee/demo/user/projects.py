...
from erpbee.demo.user.hr import make_sales_invoice_for_timesheet
...
def make_timesheet_for_projects(current_date	):
	for data in frappe.get_all("Task", ["name", "project"], {"status": "Open", "exp_end_date": ("<", current_date)}):
		...
		ts = make_timesheet(employee, simulate = True, billable = 1, company = erpbee.get_default_company(),
