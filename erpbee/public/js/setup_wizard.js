...
frappe.setup.on("before_load", function () {
	erpbee.setup.slides_settings.map(frappe.setup.add_slide);
});

erpbee.setup.slides_settings = [
	{
		// Domain
		name: 'domain',
	...
	},
	{
		...
		validate: function () {
			...
			if(me.values.bank_account){
				frappe.call({
					async: false,
					method: "erpbee.accounts.doctype.account.chart_of_accounts.chart_of_accounts.validate_bank_account",
					args: {
		...
		set_fy_dates: function (slide) {
			...
			if (country) {
				var fy = erpbee.setup.fiscal_years[country];
		...
		charts_modal: function(slide, chart_template) {
			...
			let coa_tree = new frappe.ui.Tree({
				...
				label: parent,
				expandable: true,
				method: 'erpbee.accounts.utils.get_coa',
				args: {
