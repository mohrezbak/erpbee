class CallPopup {
		...
		this.dialog = new frappe.ui.Dialog({
			...
			'fields': [{
				...
				'click': () => {
					...
					frappe.xcall('erpbee.telephony.doctype.call_log.call_log.add_call_summary', {
	...
	make_last_interaction_section() {
		frappe.xcall('erpbee.crm.doctype.utils.get_last_interaction', {
...
$(document).on('app_ready', function () {
	frappe.realtime.on('show_call_popup', call_log => {
		if (!erpbee.call_popup) {
			erpbee.call_popup = new CallPopup(call_log);
		} else {
			erpbee.call_popup.update_call_log(call_log);
			erpbee.call_popup.dialog.show();
		}
	});
});
