frappe.ui.form.on("Communication", {
	...
	make_lead_from_communication: (frm) => {
		return frappe.call({
			method: "erpbee.crm.doctype.lead.lead.make_lead_from_communication",
			args: {
	...
	make_issue_from_communication: (frm) => {
		return frappe.call({
			method: "erpbee.support.doctype.issue.issue.make_issue_from_communication",
			args: {
				communication: frm.doc.name
			},
			freeze: true,
	...
	make_opportunity_from_communication: (frm) => {
		...
		frappe.prompt(fields, data => {
			frappe.call({
				method: "erpbee.crm.doctype.opportunity.opportunity.make_opportunity_from_communication",
				args: {
					communication: frm.doc.name,
