...
erpbee.SerialNoBatchSelector = Class.extend({
	...
	make_dialog: function() {
		...
		let fields = [
			...
			},
			{
				...
				get_query: function() {
					return {
						query: "erpbee.controllers.queries.warehouse_query",
						filters: [
			...
			},
			{
				...
				click: () => {
					...
					let numbers = frappe.call({
						method: "erpbee.stock.doctype.serial_no.serial_no.auto_fetch_serial_number",
						args: {
							qty: qty,
				...
				fields: [
					{
						...
						change: function () {
							...
							if (me.warehouse_details.name) {
								frappe.call({
									method: 'erpbee.stock.doctype.batch.batch.get_batch_qty',
									args: {
										batch_no,
	...
	get_serial_no_fields: function() {
		...
		if (me.frm.doc.doctype === 'POS Invoice' && !this.showing_reserved_serial_nos_error) {
			frappe.call({
				method: "erpbee.stock.doctype.serial_no.serial_no.get_pos_reserved_serial_nos",
				args: {
					filters: {
						item_code: me.item_code,
						warehouse: typeof me.warehouse_details.name == "string" ? me.warehouse_details.name : '',
