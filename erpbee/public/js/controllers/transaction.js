...
erpbee.TransactionController = erpbee.taxes_and_totals.extend({
	setup: function() {
		this._super();
		...
		if (this.frm.fields_dict["items"].grid.get_field('blanket_order')) {
			this.frm.set_query("blanket_order", "items", function(doc, cdt, cdn) {
				...
				return {
					query: "erpbee.controllers.queries.get_blanket_orders",
					filters: {
						"company": doc.company,
	...
	make_payment_request: function() {
		...
		frappe.call({
			method:"erpbee.accounts.doctype.payment_request.payment_request.make_payment_request",
			args: {
	...
	scan_barcode: function() {
		...
		if(this.frm.doc.scan_barcode) {
			frappe.call({
				method: "erpbee.selling.page.point_of_sale.point_of_sale.search_serial_or_batch_or_barcode_number",
	...
	apply_default_taxes: function() {
		...
		if (taxes_and_charges_field) {
			return frappe.call({
				method: "erpbee.controllers.accounts_controller.get_default_taxes_and_charges",
				args: {
	...
	send_sms: function() {
		var sms_man = new erpbee.SMSManager(this.frm.doc);
	},
		...
		if(item.item_code || item.barcode || item.serial_no) {
			if(!this.validate_company_and_party()) {
				...
				return this.frm.call({
					method: "erpbee.stock.get_item_details.get_item_details",
					child: item,
					args: {
						doc: me.frm.doc,
						args: {
							...
							frappe.run_serially([
								...
								() => {
									if(show_batch_dialog && !frappe.flags.hide_serial_batch_dialog) {
										...
										erpbee.show_serial_batch_selector(me.frm, d, (item) => {
	...
	company: function() {
		...
		var set_party_account = function(set_pricing) {
			if (in_list(["Sales Invoice", "Purchase Invoice"], me.frm.doc.doctype)) {
				...
				if(party && me.frm.doc.company) {
					return frappe.call({
						method: "erpbee.accounts.party.get_party_account",
						args: {
							company: me.frm.doc.company,
		...
		if (frappe.meta.get_docfield(this.frm.doctype, "shipping_address") &&
			...
			erpbee.utils.get_shipping_address(this.frm, function(){
		...
		if(this.frm.doc.company) {
			erpbee.last_selected_company = this.frm.doc.company;
		}
	},
	...
	posting_date: function() {
		...
		if (this.frm.doc.posting_date) {
			...
			if ((this.frm.doc.doctype == "Sales Invoice" && this.frm.doc.customer) ||
				...
				return frappe.call({
					method: "erpbee.accounts.party.get_due_date",
					args: {
	...
	get_company_currency: function() {
		return erpbee.get_currency(this.frm.doc.company);
	},

	contact_person: function() {
		erpbee.utils.get_contact_details(this.frm);
	},
	...
	conversion_rate: function() {
		...
		this.frm.set_df_property("conversion_rate", "read_only", erpbee.stale_rate_allowed() ? 0 : 1);
	},
	...
	get_exchange_rate: function(transaction_date, from_currency, to_currency, callback) {
		...
		return frappe.call({
			method: "erpbee.setup.utils.get_exchange_rate",
			args: {
	...
	uom: function(doc, cdt, cdn) {
		...
		if(item.item_code && item.uom) {
			return this.frm.call({
				method: "erpbee.stock.get_item_details.get_conversion_factor",
				args: {
					item_code: item.item_code,
					uom: item.uom
				},
	...
	service_start_date: function(frm, cdt, cdn) {
		...
		if(child.service_start_date) {
			frappe.call({
				"method": "erpbee.stock.get_item_details.calculate_service_end_date",
				args: {"args": child},
				callback: function(r) {
	...
	ignore_pricing_rule: function() {
		if(this.frm.doc.ignore_pricing_rule) {
			...
			return this.frm.call({
				method: "erpbee.accounts.doctype.pricing_rule.pricing_rule.remove_pricing_rules",
	...
	apply_pricing_rule: function(item, calculate_taxes_and_totals) {
		...
		return this.frm.call({
			method: "erpbee.accounts.doctype.pricing_rule.pricing_rule.apply_pricing_rule",
	...
	apply_price_list: function(item, reset_plc_conversion) {
		...
		// `erpbee.stock.get_item_details.apply_price_list` is sensitive to its value
		...
		return this.frm.call({
			method: "erpbee.stock.get_item_details.apply_price_list",
			args: {	args: args },
			callback: function(r) {
				if (!r.exc) {
					frappe.run_serially([
	...
	get_terms: function() {
		...
		erpbee.utils.get_terms(this.frm.doc.tc_name, this.frm.doc, function(r) {
			if(!r.exc) {
	...
	taxes_and_charges: function() {
		...
		if(this.frm.doc.taxes_and_charges) {
			return this.frm.call({
				method: "erpbee.controllers.accounts_controller.get_taxes_and_charges",
				args: {
	...
	tax_category: function() {
		...
		frappe.run_serially([
			...
			() => erpbee.utils.set_taxes(this.frm, "tax_category"),
		]);
	},
	...
	item_tax_template: function(doc, cdt, cdn) {
		...
		if(item.item_tax_template) {
			return this.frm.call({
				method: "erpbee.stock.get_item_details.get_item_tax_map",
				args: {
					company: me.frm.doc.company,
					item_tax_template: item.item_tax_template,
					as_json: true
	...
	update_item_tax_map: function() {
		...
		if(item_codes.length) {
			return this.frm.call({
				method: "erpbee.stock.get_item_details.get_item_tax_info",
				args: {
					company: me.frm.doc.company,
	...
	setup_item_selector: function() {
		...
		// 	this.item_selector = new erpbee.ItemSelector({frm: this.frm});
		// }
	},
	...
	get_method_for_payment: function(){
		var method = "erpbee.accounts.doctype.payment_entry.payment_entry.get_payment_entry";
		if(cur_frm.doc.__onload && cur_frm.doc.__onload.make_payment_via_journal_entry){
			if(in_list(['Sales Invoice', 'Purchase Invoice'],  cur_frm.doc.doctype)){
				method = "erpbee.accounts.doctype.journal_entry.journal_entry.get_payment_entry_against_invoice";
			}else {
				method= "erpbee.accounts.doctype.journal_entry.journal_entry.get_payment_entry_against_order";
			}
		}
	...
	set_query_for_item_tax_template: function(doc, cdt, cdn) {
		...
		if(!item.item_code) {
			...
			return {
				query: "erpbee.controllers.queries.get_tax_template",
				filters: filters
			}
		}
	},
	...
	payment_terms_template: function() {
		...
		if(doc.payment_terms_template && doc.doctype !== 'Delivery Note') {
			...
			frappe.call({
				method: "erpbee.controllers.accounts_controller.get_payment_terms",
				args: {
	...
	payment_term: function(doc, cdt, cdn) {
		...
		if(row.payment_term) {
			frappe.call({
				method: "erpbee.controllers.accounts_controller.get_payment_term_details",
				args: {
	...
	blanket_order: function(doc, cdt, cdn) {
		...
		if (item.blanket_order && (item.parenttype=="Sales Order" || item.parenttype=="Purchase Order")) {
			frappe.call({
				method: "erpbee.stock.get_item_details.get_blanket_order_details",
				args: {
					args:{
						item_code: item.item_code,
...
erpbee.show_serial_batch_selector = function (frm, d, callback, on_close, show_dialog) {
