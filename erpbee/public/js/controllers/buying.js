...
{% include 'erpbee/accounts/doctype/purchase_taxes_and_charges_template/purchase_taxes_and_charges_template.js' %}

cur_frm.email_field = "contact_email";

erpbee.buying.BuyingController = erpbee.TransactionController.extend({
	setup: function() {
		this._super();
	},
		...
		if(this.frm.get_field('shipping_address')) {
			this.frm.set_query("shipping_address", function() {
				if(me.frm.doc.customer) {
					...
					return erpbee.queries.company_address_query(me.frm.doc)
			});
		}
		/* eslint-enable */
	},
	...
	setup_queries: function(doc, cdt, cdn) {
		...
		me.frm.set_query('supplier', erpbee.queries.supplier);
		me.frm.set_query('contact_person', erpbee.queries.contact_query);
		me.frm.set_query('supplier_address', erpbee.queries.address_query);

		me.frm.set_query('billing_address', erpbee.queries.company_address_query);

		if(this.frm.fields_dict.supplier) {
			this.frm.set_query("supplier", function() {
				return{	query: "erpbee.controllers.queries.supplier_query" }});
		}
		...
		this.frm.set_query("manufacturer", "items", function(doc, cdt, cdn) {
			...
			return {
				query: "erpbee.controllers.queries.item_manufacturer_query",
	...
	supplier: function() {
		var me = this;
		erpbee.utils.get_party_details(this.frm, null, null, function(){
			me.apply_price_list();
		});
	},

	supplier_address: function() {
		erpbee.utils.get_address_display(this.frm);
		erpbee.utils.set_taxes_from_address(this.frm, "supplier_address", "supplier_address", "supplier_address");
	},
	...
	warehouse: function(doc, cdt, cdn) {
		...
		if(item.item_code && item.warehouse) {
			return this.frm.call({
				method: "erpbee.stock.get_item_details.get_bin_details",
				child: item,
				args: {
	...
	set_from_product_bundle: function() {
		...
		this.frm.add_custom_button(__("Product Bundle"), function() {
			erpbee.buying.get_items_from_product_bundle(me.frm);
		}, __("Get Items From"));
	},

	shipping_address: function(){
		var me = this;
		erpbee.utils.get_address_display(this.frm, "shipping_address",
			"shipping_address_display", true);
	},

	billing_address: function() {
		erpbee.utils.get_address_display(this.frm, "billing_address",
	...
	manufacturer: function(doc, cdt, cdn) {
		...
		if(row.manufacturer) {
			frappe.call({
				method: "erpbee.stock.doctype.item_manufacturer.item_manufacturer.get_item_manufacturer_part_no",
				args: {
...
erpbee.buying.link_to_mrs = function(frm) {
	frappe.call({
...
erpbee.buying.get_default_bom = function(frm) {
	$.each(frm.doc["items"] || [], function(i, d) {
		if (d.item_code && d.bom === "") {
			return frappe.call({
				type: "GET",
				method: "erpbee.stock.get_item_details.get_default_bom",
				args: {
...
erpbee.buying.get_items_from_product_bundle = function(frm) {
	var dialog = new frappe.ui.Dialog({
		...
		primary_action(args){
			...
			return frappe.call({
				type: "GET",
				method: "erpbee.stock.doctype.packed_item.packed_item.get_items_from_product_bundle",
				args: {
					args: {
