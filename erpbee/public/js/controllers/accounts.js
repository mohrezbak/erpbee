...
// get tax rate
frappe.provide("erpbee.taxes");
frappe.provide("erpbee.taxes.flags");
...
frappe.ui.form.on(cur_frm.doctype, {
	setup: function(frm) {
		...
		$(frm.wrapper).on('grid-row-render', function(e, grid_row) {
			if(in_list(['Sales Taxes and Charges', 'Purchase Taxes and Charges'], grid_row.doc.doctype)) {
				erpbee.taxes.set_conditional_mandatory_rate_or_amount(grid_row);
			}
		});
	},
	onload: function(frm) {
	...
	taxes_on_form_rendered: function(frm) {
		erpbee.taxes.set_conditional_mandatory_rate_or_amount(frm.open_grid_row());
	},
...
var get_payment_mode_account = function(frm, mode_of_payment, callback) {
	...
	return  frappe.call({
		method: "erpbee.accounts.doctype.sales_invoice.sales_invoice.get_bank_cash_account",
		args: {
...
cur_frm.cscript.account_head = function(doc, cdt, cdn) {
	...
	if(!d.charge_type && d.account_head){
		...
		frappe.call({
			type:"GET",
			method: "erpbee.controllers.accounts_controller.get_tax_rate",
...
if(!erpbee.taxes.flags[cur_frm.cscript.tax_table]) {
	erpbee.taxes.flags[cur_frm.cscript.tax_table] = true;
	...
	frappe.ui.form.on(cur_frm.cscript.tax_table, "charge_type", function(frm, cdt, cdn) {
		...
		if(open_form) {
			erpbee.taxes.set_conditional_mandatory_rate_or_amount(open_form);
		} else {
			// apply in current row
			erpbee.taxes.set_conditional_mandatory_rate_or_amount(frm.get_field('taxes').grid.get_row(cdn));
		}
	});
...
erpbee.taxes.set_conditional_mandatory_rate_or_amount = function(grid_row) {
	if(grid_row) {
