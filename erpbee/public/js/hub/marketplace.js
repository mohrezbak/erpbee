...
frappe.utils.make_event_emitter(erpbee.hub);

erpbee.hub.Marketplace = class Marketplace {
	constructor({ parent }) {
		this.$parent = $(parent);
	...
	make_body() {
		...
		if (!hub.is_server) {
			erpbee.hub.on('seller-registered', () => {
	...
	register_marketplace({company, company_description}) {
		frappe.call({
		    method: 'erpbee.hub_node.api.register_marketplace',
		    args: {
				company,
				company_description
			}
			...
			if (r.message && r.message.ok) {
				...
				this.update_hub_settings()
					.then(() => {
						...
						erpbee.hub.trigger('seller-registered');
					});
			}
		});
	}
			...
			.then(r => {
				...
				const d = new frappe.ui.Dialog({
					...
					primary_action({ users }) {
						...
						frappe.call('erpbee.hub_node.api.register_users', {
	...
	get_unregistered_users() {
		return frappe.call('erpbee.hub_node.api.get_unregistered_users')
	}
