...
$.extend(erpbee, {
	...
	hide_company: function() {
		if(cur_frm.fields_dict.company) {
			...
			} else if(erpbee.last_selected_company) {
				if(!cur_frm.doc.company) cur_frm.set_value("company", erpbee.last_selected_company);
			}
		}
	},
...
$.extend(erpbee.utils, {
		...
		if(frm.doc.__onload && frm.doc.__onload.dashboard_info) {
			...
			if(company_wise_info.length > 1) {
				company_wise_info.forEach(function(info) {
					erpbee.utils.add_indicator_for_multicompany(frm, info);
				});
	...
	get_terms: function(tc_name, doc, callback) {
		if(tc_name) {
			return frappe.call({
				method: 'erpbee.setup.doctype.terms_and_conditions.terms_and_conditions.get_terms_and_conditions',
				args: {
	...
	make_bank_account: function(doctype, docname) {
		frappe.call({
			method: "erpbee.accounts.doctype.bank_account.bank_account.make_bank_account",
			args: {
				doctype: doctype,
				docname: docname
			},
	...
	add_dimensions: function(report_name, index) {
		...
		erpbee.dimension_filters.forEach((dimension) => {
	...
	make_pricing_rule: function(doctype, docname) {
		frappe.call({
			method: "erpbee.accounts.doctype.pricing_rule.pricing_rule.make_pricing_rule",
			args: {
				doctype: doctype,
				docname: docname
			},
...
erpbee.utils.select_alternate_items = function(opts) {
	const frm = opts.frm;
		...
		fields: [
			{fieldtype:'Section Break', label: __('Items')},
			{
				...
				fields: [{
					...
					get_query: (e) => {
						return {
							query: "erpbee.stock.doctype.item_alternative.item_alternative.get_alternative_items",
							filters: {
...
erpbee.utils.update_child_items = function(opts) {
	const frm = opts.frm;
	...
	const dialog = new frappe.ui.Dialog({
		...
		primary_action: function() {
			...
			frappe.call({
				method: 'erpbee.controllers.accounts_controller.update_child_qty_rate',
				freeze: true,
				args: {
...
erpbee.utils.map_current_doc = function(opts) {
	let query_args = {};
