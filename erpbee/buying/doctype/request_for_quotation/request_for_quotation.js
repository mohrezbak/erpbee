...
frappe.ui.form.on("Request for Quotation",{
	setup: function(frm) {
		...
		frm.fields_dict["suppliers"].grid.get_field("contact").get_query = function(doc, cdt, cdn) {
			...
			return {
				query: "erpbee.buying.doctype.request_for_quotation.request_for_quotation.get_supplier_contacts",
	...
	refresh: function(frm, cdt, cdn) {
		if (frm.doc.docstatus === 1) {
			...
			frm.add_custom_button(__("Send Emails to Suppliers"), function() {
				frappe.call({
					method: 'erpbee.buying.doctype.request_for_quotation.request_for_quotation.send_supplier_emails',
					freeze: true,
					args: {
			...
			frm.add_custom_button(__('Download PDF'), () => {
				...
				frappe.prompt(fields, data => {
					...
					var w = window.open(
						frappe.urllib.get_full_url("/api/method/erpbee.buying.doctype.request_for_quotation.request_for_quotation.get_pdf?"
	...
	make_suppplier_quotation: function(frm) {
		...
		var dialog = new frappe.ui.Dialog({
			...
			primary_action: (args) => {
				...
				return frappe.call({
					type: "GET",
					method: "erpbee.buying.doctype.request_for_quotation.request_for_quotation.make_supplier_quotation_from_rfq",
					args: {
...
erpbee.buying.RequestforQuotationController = erpbee.buying.BuyingController.extend({
	refresh: function() {
		var me = this;
		this._super();
		if (this.frm.doc.docstatus===0) {
			...
			this.frm.add_custom_button(__('Opportunity'),
				function() {
					erpbee.utils.map_current_doc({
						method: "erpbee.crm.doctype.opportunity.opportunity.make_request_for_quotation",
			...
			this.frm.add_custom_button(__('Possible Supplier'), function() {
				...
				var dialog = new frappe.ui.Dialog({
					...
					primary_action: (args) => {
						...
						erpbee.utils.map_current_doc({
							method: "erpbee.buying.doctype.request_for_quotation.request_for_quotation.get_item_from_material_requests_based_on_supplier",
			...
			this.frm.add_custom_button(__('Link to Material Requests'),
				function() {
					erpbee.buying.link_to_mrs(me.frm);
				}, __("Tools"));
			...
			fields: [
				{
					...
					onchange() {
						if(dialog.get_value('search_type') == 'Tag'){
							frappe.call({
								method: 'erpbee.buying.doctype.request_for_quotation.request_for_quotation.get_supplier_tag',
							}).then(r => {
...
$.extend(cur_frm.cscript, new erpbee.buying.RequestforQuotationController({frm: cur_frm}));
