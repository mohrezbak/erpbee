...
frappe.provide("erpbee.buying");

{% include 'erpbee/public/js/controllers/buying.js' %};
...
frappe.ui.form.on("Purchase Order", {
	setup: function(frm) {
		...
		frm.set_query("expense_account", "items", function() {
			return {
				query: "erpbee.controllers.queries.get_expense_account",
	...
	onload: function(frm) {
		...
		erpbee.queries.setup_queries(frm, "Warehouse", function() {
			return erpbee.queries.warehouse(frm.doc);
		});
	}
});
	...
	schedule_date: function(frm, cdt, cdn) {
		...
		if (row.schedule_date) {
			if(!frm.doc.schedule_date) {
				erpbee.utils.copy_value_in_all_rows(frm.doc, cdt, cdn, "items", "schedule_date");
			} else {
				set_schedule_date(frm);
			}
		}
...
erpbee.buying.PurchaseOrderController = erpbee.buying.BuyingController.extend({
	setup: function() {
			...
			if(!in_list(["Closed", "Delivered"], doc.status)) {
				if(this.frm.doc.status !== 'Closed' && flt(this.frm.doc.per_received) < 100 && flt(this.frm.doc.per_billed) < 100) {
					this.frm.add_custom_button(__('Update Items'), () => {
						erpbee.utils.update_child_items({
							frm: this.frm,
							child_docname: "items",
	...
	get_items_from_open_material_requests: function() {
		erpbee.utils.map_current_doc({
			method: "erpbee.stock.doctype.material_request.material_request.make_purchase_order_based_on_supplier",
			args: {
			...
			get_query_method: "erpbee.stock.doctype.material_request.material_request.get_material_requests_based_on_supplier"
		});
	},
	...
	_make_rm_stock_entry: function(rm_items) {
		frappe.call({
			method:"erpbee.buying.doctype.purchase_order.purchase_order.make_rm_stock_entry",
			args: {
	...
	make_inter_company_order: function(frm) {
		frappe.model.open_mapped_doc({
			method: "erpbee.buying.doctype.purchase_order.purchase_order.make_inter_company_sales_order",
			frm: frm
		});
	},
	...
	make_purchase_receipt: function() {
		frappe.model.open_mapped_doc({
			method: "erpbee.buying.doctype.purchase_order.purchase_order.make_purchase_receipt",
			frm: cur_frm
		})
	},
	...
	make_purchase_invoice: function() {
		frappe.model.open_mapped_doc({
			method: "erpbee.buying.doctype.purchase_order.purchase_order.make_purchase_invoice",
			frm: cur_frm
		})
	},
	...
	add_from_mappers: function() {
		...
		this.frm.add_custom_button(__('Material Request'),
			function() {
				erpbee.utils.map_current_doc({
					method: "erpbee.stock.doctype.material_request.material_request.make_purchase_order",
					source_doctype: "Material Request",
					target: me.frm,
					setters: {
		...
		this.frm.add_custom_button(__('Supplier Quotation'),
			function() {
				erpbee.utils.map_current_doc({
					method: "erpbee.buying.doctype.supplier_quotation.supplier_quotation.make_purchase_order",
		...
		function() {
			...
			frappe.call({
				method: "erpbee.buying.utils.get_linked_material_requests",
				args:{
					items: my_items
				},
				callback: function(r) {
...
$.extend(cur_frm.cscript, new erpbee.buying.PurchaseOrderController({frm: cur_frm}));

cur_frm.cscript.update_status= function(label, status){
	frappe.call({
		method: "erpbee.buying.doctype.purchase_order.purchase_order.update_status",
...
function set_schedule_date(frm) {
	if(frm.doc.schedule_date){
		erpbee.utils.copy_value_in_all_rows(frm.doc, frm.doc.doctype, frm.doc.name, "items", "schedule_date");
	}
}
...
frappe.ui.form.on("Purchase Order", "is_subcontracted", function(frm) {
	if (frm.doc.is_subcontracted === "Yes") {
		erpbee.buying.get_default_bom(frm);
	}
});
