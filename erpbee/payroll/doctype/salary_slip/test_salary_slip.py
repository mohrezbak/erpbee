...
from erpbee.payroll.doctype.payroll_entry.payroll_entry import get_month_details
from erpbee.hr.doctype.employee.test_employee import make_employee
from erpbee.hr.doctype.leave_allocation.test_leave_allocation import create_leave_allocation
from erpbee.hr.doctype.leave_type.test_leave_type import create_leave_type
from erpbee.payroll.doctype.employee_tax_exemption_declaration.test_employee_tax_exemption_declaration \
...
class TestSalarySlip(unittest.TestCase):
	...
	def test_payroll_frequency(self):
		fiscal_year = get_fiscal_year(nowdate(), company=erpbee.get_default_company())[0]
	...
	def test_tax_for_payroll_period(self):
		...
		from erpbee.payroll.doctype.salary_structure.test_salary_structure import \
...
def make_leave_application(employee, from_date, to_date, leave_type, company=None):
	leave_application = frappe.get_doc(dict(
		...
		from_date = from_date,
		to_date = to_date,
		company = company or erpbee.get_default_company() or "_Test Company",
		docstatus = 1,
		status = "Approved",
...
def setup_test():
	...
	frappe.db.set_value("Company", erpbee.get_default_company(), "default_holiday_list", "Salary Slip Test Holiday List")
	frappe.db.set_value("Payroll Settings", None, "email_salary_slip_to_employee", 0)
	frappe.db.set_value('HR Settings', None, 'leave_status_notification_template', None)
	frappe.db.set_value('HR Settings', None, 'leave_approval_notification_template', None)

def make_holiday_list():
	fiscal_year = get_fiscal_year(nowdate(), company=erpbee.get_default_company())
