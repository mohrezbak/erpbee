...
frappe.ui.form.on("Timesheet", {
	setup: function(frm) {
		frappe.require("/assets/erpbee/js/projects/timer.js");
			...
			frm.add_custom_button(__(button), function() {
				...
				$.each(frm.doc.time_logs || [], function(i, row) {
					...
					if (flag && row.activity_type && !row.from_time){
						erpbee.timesheet.timer(frm, row);
					...
					if (flag && row.from_time <= frappe.datetime.now_datetime() && !row.completed) {
						...
						erpbee.timesheet.timer(frm, row, timestamp);
						flag = false;
					}
				});
				// If no activities found to start a timer, create new
				if (flag) {
					erpbee.timesheet.timer(frm);
				}
	...
	make_invoice: function(frm) {
		...
		dialog.set_primary_action(__('Create Sales Invoice'), () => {
			...
			return frappe.call({
				type: "GET",
				method: "erpbee.projects.doctype.timesheet.timesheet.make_sales_invoice",
				args: {
	...
	make_salary_slip: function(frm) {
		frappe.model.open_mapped_doc({
			method: "erpbee.projects.doctype.timesheet.timesheet.make_salary_slip",
			frm: frm
		});
	},
...
frappe.ui.form.on("Timesheet Detail", {
	...
	activity_type: function(frm, cdt, cdn) {
		frappe.call({
			method: "erpbee.projects.doctype.timesheet.timesheet.get_activity_cost",
			args: {
...
function set_project_in_timelog(frm) {
	if(frm.doc.project){
		erpbee.utils.copy_value_in_all_rows(frm.doc, frm.doc.doctype, frm.doc.name, "time_logs", "project");
	}
}