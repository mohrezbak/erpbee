erpbee.setup_einvoice_actions = (doctype) => {
	frappe.ui.form.on(doctype, {
		refresh(frm) {
			...
			if (!irn && !__unsaved) {
				const action = () => {
					frappe.call({
						method: 'erpbee.regional.india.e_invoice.utils.get_einvoice',
				...
				const action = () => {
					const d = new frappe.ui.Dialog({
						...
						primary_action: function() {
							...
							frappe.call({
								method: 'erpbee.regional.india.e_invoice.utils.cancel_irn',
								args: { 
									doctype,
									docname: name,
									irn: irn,
				...
				const action = () => {
					const d = new frappe.ui.Dialog({
						...
						primary_action: function() {
							const data = d.get_values();
							frappe.call({
								method: 'erpbee.regional.india.e_invoice.utils.generate_eway_bill',
								args: {
									doctype,
									docname: name,
									irn,
				...
				const action = () => {
					const d = new frappe.ui.Dialog({
						...
						primary_action: function() {
							const data = d.get_values();
							frappe.call({
								method: 'erpbee.regional.india.e_invoice.utils.cancel_eway_bill',
								args: {
									doctype,
									docname: name,
									eway_bill: ewaybill,
...
const request_irn_generation = (frm) => {
	frappe.call({
		method: 'erpbee.regional.india.e_invoice.utils.generate_irn',
