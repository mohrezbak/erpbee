frappe.ui.form.on("Issue", {
	onload: function(frm) {
		...
		if (frm.doc.service_level_agreement) {
			frappe.call({
				method: "erpbee.support.doctype.service_level_agreement.service_level_agreement.get_service_level_agreement_filters",
				args: {
		...
		if (frm.doc.status !== "Closed" && frm.doc.agreement_status === "Ongoing") {
			...
			frm.add_custom_button(__("Task"), function () {
				frappe.model.open_mapped_doc({
					method: "erpbee.support.doctype.issue.issue.make_task",
					frm: frm
				});
			}, __("Create"));
