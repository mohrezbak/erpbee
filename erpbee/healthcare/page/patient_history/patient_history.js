	...
	this.page.main.on("click", ".btn-more", function() {
		...
		if(me.page.main.find("."+docname).parent().find('.document-html').attr('data-fetched') == "1"){
			...
			if(doctype && docname){
				...
				frappe.call({
					method: "erpbee.healthcare.utils.render_doc_as_html",
					args:{
						doctype: doctype,
						docname: docname,
...
var get_documents = function(patient, me){
	frappe.call({
		"method": "erpbee.healthcare.page.patient_history.patient_history.get_feed",
		args: {
			name: patient,
			start: me.start,
			page_length: 20
...
var show_patient_vital_charts = function(patient, me, btn_show_id, pts, title) {
	frappe.call({
		method: "erpbee.healthcare.utils.get_patient_vitals",
		args:{
			patient: patient
		},
		callback: function(r) {
