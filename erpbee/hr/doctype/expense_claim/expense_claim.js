...
frappe.provide("erpbee.hr");

erpbee.hr.ExpenseClaimController = frappe.ui.form.Controller.extend({
	expense_type: function(doc, cdt, cdn) {
		...
		return frappe.call({
			method: "erpbee.hr.doctype.expense_claim.expense_claim.get_expense_claim_account_and_cost_center",
			args: {
...
$.extend(cur_frm.cscript, new erpbee.hr.ExpenseClaimController({frm: cur_frm}));
...
frappe.ui.form.on("Expense Claim", {
	setup: function(frm) {
		...
		frm.set_query("expense_approver", function() {
			return {
				query: "erpbee.hr.doctype.department_approver.department_approver.get_approvers",
				filters: {
					employee: frm.doc.employee,
	...
	onload: function(frm) {
		if (frm.doc.docstatus == 0) {
			return frappe.call({
				method: "erpbee.hr.doctype.leave_application.leave_application.get_mandatory_approval",
				args: {
					doctype: frm.doc.doctype,
				},
				callback: function(r) {
	...
	make_payment_entry: function(frm) {
		...
		if(frm.doc.__onload && frm.doc.__onload.make_payment_via_journal_entry) {
			method = "erpbee.hr.doctype.expense_claim.expense_claim.make_bank_entry";
		}
		return frappe.call({
			method: method,
			args: {
	...
	employee_name: function(frm) {
		erpbee.expense_claim.set_title(frm);
	},
	...
	get_advances: function(frm) {
		...
		if (frm.doc.employee) {
			return frappe.call({
				method: "erpbee.hr.doctype.expense_claim.expense_claim.get_advances",
				args: {
					employee: frm.doc.employee
				},
...
frappe.ui.form.on("Expense Claim Detail", {
	...
	cost_center: function(frm, cdt, cdn) {
		erpbee.utils.copy_value_in_all_rows(frm.doc, cdt, cdn, "expenses", "cost_center");
	}
});
