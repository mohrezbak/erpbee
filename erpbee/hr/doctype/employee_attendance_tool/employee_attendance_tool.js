frappe.ui.form.on("Employee Attendance Tool", {
	...
	onload: function(frm) {
		...
		erpbee.employee_attendance_tool.load_employees(frm);
	},
...
erpbee.employee_attendance_tool = {
	load_employees: function(frm) {
		if(frm.doc.date) {
			frappe.call({
				method: "erpbee.hr.doctype.employee_attendance_tool.employee_attendance_tool.get_employees",
				args: {
					date: frm.doc.date,
				...
				callback: function(r) {
					if(r.message['unmarked'].length > 0) {
						...
						frm.EmployeeSelector = new erpbee.EmployeeSelector(frm, frm.employee_area, r.message['unmarked'])
					}
					else{
					...
					if(r.message['marked'].length > 0) {
						...
						frm.marked_employee = new erpbee.MarkedEmployee(frm, frm.marked_employee_area, r.message['marked'])
					}
					else{
...
erpbee.MarkedEmployee = Class.extend({
...
erpbee.EmployeeSelector = Class.extend({
	init: function(frm, wrapper, employee) {
		this.wrapper = wrapper;
		this.frm = frm;
		this.make(frm, employee);
	...
	make: function(frm, employee) {
		...
		mark_employee_toolbar.find(".btn-mark-present")
			...
			.on("click", function() {
				...
				frappe.call({
					method: "erpbee.hr.doctype.employee_attendance_tool.employee_attendance_tool.mark_employee_attendance",
					args:{
