...
import frappe
from erpbee import encode_company_abbr
from erpbee.accounts.doctype.account.chart_of_accounts.chart_of_accounts import create_charts
from erpbee.accounts.doctype.chart_of_accounts_importer.chart_of_accounts_importer import unset_existing_data
...
class TallyMigration(Document):
	...
	def set_account_defaults(self):
		self.default_cost_center, self.default_round_off_account = frappe.db.get_value("Company", self.erpbee_company, ["cost_center", "round_off_account"])
		...
		def get_stock_items_uoms(collection):
			...
			for item in collection.find_all("STOCKITEM"):
				...
				items.append({
					...
					"item_defaults": [{"company": self.erpbee_company}]
				})
		...
		def create_company_and_coa(coa_file_url):
			...
			try:
				company = frappe.get_doc({
					"doctype": "Company",
					"company_name": self.erpbee_company,
					"default_currency": "INR",
					"enable_perpetual_inventory": 0,
				}).insert()
			except frappe.DuplicateEntryError:
				company = frappe.get_doc("Company", self.erpbee_company)
				unset_existing_data(self.erpbee_company)
	...
	def _process_day_book_data(self):
		...
		def voucher_to_journal_entry(voucher):
			...
			for entry in ledger_entries:
				account = {"account": encode_company_abbr(entry.LEDGERNAME.string.strip(), self.erpbee_company), "cost_center": self.default_cost_center}
		...
		def voucher_to_invoice(voucher):
			if voucher.VOUCHERTYPENAME.string.strip() in ["Sales", "Credit Note"]:
				...
				account_name = encode_company_abbr(self.tally_debtors_account, self.erpbee_company)
				price_list_field = "selling_price_list"
			elif voucher.VOUCHERTYPENAME.string.strip() in ["Purchase", "Debit Note"]:
				doctype = "Purchase Invoice"
				party_field = "supplier"
				account_field = "credit_to"
				account_name = encode_company_abbr(self.tally_creditors_account, self.erpbee_company)
		...
		def get_voucher_items(voucher, doctype):
			...
			for entry in inventory_entries:
				...
				items.append({
					...
					account_field: encode_company_abbr(entry.find_all("ACCOUNTINGALLOCATIONS.LIST")[0].LEDGERNAME.string.strip(), self.erpbee_company),
				})
			return items
		...
		def get_voucher_taxes(voucher):
			...
			for entry in ledger_entries:
				if entry.ISPARTYLEDGER.string.strip() == "No":
					tax_account = encode_company_abbr(entry.LEDGERNAME.string.strip(), self.erpbee_company)
					taxes.append({
						"charge_type": "Actual",
		...
		def get_party(party):
			if frappe.db.exists({"doctype": "Supplier", "supplier_name": party}):
				return "Supplier", encode_company_abbr(self.tally_creditors_account, self.erpbee_company)
			elif frappe.db.exists({"doctype": "Customer", "customer_name": party}):
				return "Customer", encode_company_abbr(self.tally_debtors_account, self.erpbee_company)
	...
	def _import_day_book_data(self):
		...
		try:
			frappe.db.set_value("Account", encode_company_abbr(self.tally_creditors_account, self.erpbee_company), "account_type", "Payable")
			frappe.db.set_value("Account", encode_company_abbr(self.tally_debtors_account, self.erpbee_company), "account_type", "Receivable")
			frappe.db.set_value("Company", self.erpbee_company, "round_off_account", self.default_round_off_account)
