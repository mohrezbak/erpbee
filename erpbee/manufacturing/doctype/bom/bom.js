...
frappe.ui.form.on("BOM", {
	setup: function(frm) {
		...
		frm.set_query("item", function() {
			return {
				query: "erpbee.manufacturing.doctype.bom.bom.item_query"
			};
		});
	...
	make_work_order: function(frm) {
		frm.events.setup_variant_prompt(frm, "Work Order", (frm, item, data, variant_items) => {
			frappe.call({
				method: "erpbee.manufacturing.doctype.work_order.work_order.make_work_order",
				args: {
					bom_no: frm.doc.name,
					item: item,
					qty: data.qty || 0.0,
	...
	make_variant_bom: function(frm) {
		frm.events.setup_variant_prompt(frm, "Variant BOM", (frm, item, data, variant_items) => {
			frappe.call({
				method: "erpbee.manufacturing.doctype.bom.bom.make_variant_bom",
				args: {
	...
	make_quality_inspection: function(frm) {
		frappe.model.open_mapped_doc({
			method: "erpbee.stock.doctype.quality_inspection.quality_inspection.make_quality_inspection",
			frm: frm
		})
	},
		...
		if (frm.doc.routing) {
			frappe.call({
				...
				callback: function(r) {
					if (!r.exc) {
						frm.refresh_fields();
						erpbee.bom.calculate_op_cost(frm.doc);
						erpbee.bom.calculate_total(frm.doc);
					}
				}
			});
		}
...
erpbee.bom.BomController = erpbee.TransactionController.extend({
	conversion_rate: function(doc) {
		if(this.frm.doc.currency === this.get_company_currency()) {
			this.frm.set_value("conversion_rate", 1.0);
		} else {
			erpbee.bom.update_cost(doc);
		}
	},
...
$.extend(cur_frm.cscript, new erpbee.bom.BomController({frm: cur_frm}));

cur_frm.cscript.hour_rate = function(doc) {
	erpbee.bom.calculate_op_cost(doc);
	erpbee.bom.calculate_total(doc);
};
	...
	if (d.item_code) {
		return frappe.call({
			...
			callback: function(r) {
				...
				erpbee.bom.calculate_rm_cost(doc);
				erpbee.bom.calculate_scrap_materials_cost(doc);
				erpbee.bom.calculate_total(doc);
			},
			freeze: true
		});
...
erpbee.bom.update_cost = function(doc) {
	erpbee.bom.calculate_op_cost(doc);
	erpbee.bom.calculate_rm_cost(doc);
	erpbee.bom.calculate_scrap_materials_cost(doc);
	erpbee.bom.calculate_total(doc);
...
erpbee.bom.calculate_op_cost = function(doc) {
...
// rm : raw material
erpbee.bom.calculate_rm_cost = function(doc) {
	var rm = doc.items || [];
	var total_rm_cost = 0;
...
// sm : scrap material
erpbee.bom.calculate_scrap_materials_cost = function(doc) {
...
erpbee.bom.calculate_total = function(doc) {
...
frappe.ui.form.on("BOM Item", "items_remove", function(frm) {
	erpbee.bom.calculate_rm_cost(frm.doc);
	erpbee.bom.calculate_total(frm.doc);
});
