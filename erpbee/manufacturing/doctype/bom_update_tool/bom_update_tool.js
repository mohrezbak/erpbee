...
frappe.ui.form.on('BOM Update Tool', {
	...
	replace: function(frm) {
		if (frm.doc.current_bom && frm.doc.new_bom) {
			frappe.call({
				method: "erpbee.manufacturing.doctype.bom_update_tool.bom_update_tool.enqueue_replace_bom",
				freeze: true,
				args: {
					args: {
	...
	update_latest_price_in_all_boms: function() {
		frappe.call({
			method: "erpbee.manufacturing.doctype.bom_update_tool.bom_update_tool.enqueue_update_cost",
			freeze: true,
			callback: function() {
