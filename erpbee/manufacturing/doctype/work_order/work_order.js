...
frappe.ui.form.on("Work Order", {
	setup: function(frm) {
		...
		frm.set_query("bom_no", function() {
			if (frm.doc.production_item) {
				return {
					query: "erpbee.controllers.queries.bom",
		...
		frm.set_query("production_item", function() {
			return {
				query: "erpbee.controllers.queries.item_query",
				filters:[
					['is_stock_item', '=',1]
				]
			};
		...
		frm.set_query("operation", "required_items", function() {
			return {
				query: "erpbee.manufacturing.doctype.work_order.work_order.get_bom_operations",
				filters: {
	...
	onload: function(frm) {
		...
		if(frm.doc.__islocal) {
			...
			erpbee.work_order.set_default_warehouse(frm);
		}
	},
	...
	refresh: function(frm) {
		...
		erpbee.work_order.set_custom_buttons(frm);
		frm.set_intro("");
	...
	make_job_card: function(frm) {
		...
		const dialog = frappe.prompt({fieldname: 'operations', fieldtype: 'Table', label: __('Operations'),
			...
			frappe.call({
				method: "erpbee.manufacturing.doctype.work_order.work_order.make_job_card",
				args: {
	...
	production_item: function(frm) {
		if (frm.doc.production_item) {
			frappe.call({
				method: "erpbee.manufacturing.doctype.work_order.work_order.get_item_details",
				args: {
				...
				callback: function(r) {
					if(r.message) {
						...
						erpbee.in_production_item_onchange = true;
						...
						erpbee.in_production_item_onchange = false;
					}
				}
			});
		}
	...
	project: function(frm) {
		if(!erpbee.in_production_item_onchange && !frm.doc.bom_no) {
	...
	set_sales_order: function(frm) {
		if(frm.doc.production_item) {
			frappe.call({
				method: "erpbee.manufacturing.doctype.work_order.work_order.query_sales_order",
	...
	additional_operating_cost: function(frm) {
		erpbee.work_order.calculate_cost(frm.doc);
		erpbee.work_order.calculate_total_cost(frm);
	}
});
...
frappe.ui.form.on("Work Order Item", {
	source_warehouse: function(frm, cdt, cdn) {
		...
		if(!row.item_code) {
			...
			frappe.call({
				"method": "erpbee.stock.utils.get_latest_stock_qty",
				args: {
	...
	item_code: function(frm, cdt, cdn) {
		...
		if (row.item_code) {
			frappe.call({
				method: "erpbee.stock.doctype.item.item.get_item_details",
				args: {
					item_code: row.item_code,
					company: frm.doc.company
				},
...
erpbee.work_order = {
		...
		if (doc.docstatus === 1) {
			if (doc.status != 'Stopped' && doc.status != 'Completed') {
				frm.add_custom_button(__('Stop'), function() {
					erpbee.work_order.stop_work_order(frm, "Stopped");
				}, __("Status"));
			} else if (doc.status == 'Stopped') {
				frm.add_custom_button(__('Re-open'), function() {
					erpbee.work_order.stop_work_order(frm, "Resumed");
				}, __("Status"));
			}
			...
			if (show_start_btn) {
				if ((flt(doc.material_transferred_for_manufacturing) < flt(doc.qty))
					...
					frm.add_custom_button(__('Create Pick List'), function() {
						erpbee.work_order.create_pick_list(frm);
					});
					var start_btn = frm.add_custom_button(__('Start'), function() {
						erpbee.work_order.make_se(frm, 'Material Transfer for Manufacture');
					});
			...
			if(!frm.doc.skip_transfer){
				...
				&& frm.doc.status != 'Stopped') {
					...
					if (frm.doc.__onload && frm.doc.__onload.material_consumption == 1) {
						...
						if (counter > 0) {
							var consumption_btn = frm.add_custom_button(__('Material Consumption'), function() {
								...
								erpbee.work_order.make_consumption_se(frm, backflush_raw_materials_based_on);
							});
					...
					var finish_btn = frm.add_custom_button(__('Finish'), function() {
						erpbee.work_order.make_se(frm, 'Manufacture');
					});
	...
	set_default_warehouse: function(frm) {
		if (!(frm.doc.wip_warehouse || frm.doc.fg_warehouse)) {
			frappe.call({
				method: "erpbee.manufacturing.doctype.work_order.work_order.get_default_warehouse",
				callback: function(r) {
					if (!r.exe) {
	...
	make_se: function(frm, purpose) {
		this.show_prompt_for_qty_input(frm, purpose)
			.then(data => {
				return frappe.xcall('erpbee.manufacturing.doctype.work_order.work_order.make_stock_entry', {
	...
	create_pick_list: function(frm, purpose='Material Transfer for Manufacture') {
		this.show_prompt_for_qty_input(frm, purpose)
			.then(data => {
				return frappe.xcall('erpbee.manufacturing.doctype.work_order.work_order.create_pick_list', {
	...
	make_consumption_se: function(frm, backflush_raw_materials_based_on) {
		...
		frappe.call({
			method:"erpbee.manufacturing.doctype.work_order.work_order.make_stock_entry",
			args: {
				"work_order_id": frm.doc.name,
	...
	stop_work_order: function(frm, status) {
		frappe.call({
			method: "erpbee.manufacturing.doctype.work_order.work_order.stop_unstop",
			args: {
				work_order: frm.doc.name,
				status: status
			},
