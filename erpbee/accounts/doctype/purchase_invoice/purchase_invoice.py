...
import frappe, erpbee
from frappe.utils import cint, cstr, formatdate, flt, getdate, nowdate, get_link_to_form
from frappe import _, throw
import frappe.defaults

from erpbee.assets.doctype.asset_category.asset_category import get_asset_category_account
from erpbee.controllers.buying_controller import BuyingController
from erpbee.accounts.party import get_party_account, get_due_date
from erpbee.accounts.utils import get_account_currency, get_fiscal_year
from erpbee.stock.doctype.purchase_receipt.purchase_receipt import update_billed_amount_based_on_po
from erpbee.stock import get_warehouse_account_map
from erpbee.accounts.general_ledger import make_gl_entries, merge_similar_entries, make_reverse_gl_entries
from erpbee.accounts.doctype.gl_entry.gl_entry import update_outstanding_amt
from erpbee.buying.utils import check_on_hold_or_closed_status
from erpbee.accounts.general_ledger import get_round_off_account_and_cost_center
from erpbee.assets.doctype.asset.asset import get_asset_account, is_cwip_accounting_enabled
from frappe.model.mapper import get_mapped_doc
from six import iteritems
from erpbee.accounts.doctype.sales_invoice.sales_invoice import validate_inter_company_party, update_linked_doc,\
	unlink_inter_company_doc
from erpbee.accounts.doctype.tax_withholding_category.tax_withholding_category import get_party_tax_withholding_details
from erpbee.accounts.deferred_revenue import validate_service_stop_date
from erpbee.stock.doctype.purchase_receipt.purchase_receipt import get_item_account_wise_additional_cost
...
class PurchaseInvoice(BuyingController):
	...
	def check_conversion_rate(self):
		default_currency = erpbee.get_company_currency(self.company)
	...
	def set_expense_account(self, for_validate=False):
		auto_accounting_for_stock = erpbee.is_perpetual_inventory_enabled(self.company)
	...
	def on_submit(self):
		...
		if self.update_stock == 1:
			...
			from erpbee.stock.doctype.serial_no.serial_no import update_serial_nos_after_submit
	...
	def get_gl_entries(self, warehouse_account=None):
		self.auto_accounting_for_stock = erpbee.is_perpetual_inventory_enabled(self.company)
		...
		for item in self.get("items"):
			if item.is_fixed_asset:
				...
				if not self.update_stock:
					...
					if item.item_tax_amount and not cint(erpbee.is_perpetual_inventory_enabled(self.company)):
						asset_eiiav_currency = get_account_currency(eiiav_account)
						gl_entries.append(self.get_gl_dict({
							"account": eiiav_account,
							"against": self.supplier,
	...
	def update_billing_status_in_pr(self, update_modified=True):
		...
		for pr in set(updated_pr):
			from erpbee.stock.doctype.purchase_receipt.purchase_receipt import update_billing_percentage
...
def get_list_context(context=None):
	from erpbee.controllers.website_list_for_contact import get_list_context
...
def make_inter_company_sales_invoice(source_name, target_doc=None):
	from erpbee.accounts.doctype.sales_invoice.sales_invoice import make_inter_company_transaction
