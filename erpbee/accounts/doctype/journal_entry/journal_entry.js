...
frappe.provide("erpbee.journal_entry");
...
frappe.ui.form.on("Journal Entry", {
	...
	refresh: function(frm) {
		...
		if(frm.doc.docstatus==1) {
			frm.add_custom_button(__('Reverse Journal Entry'), function() {
				return erpbee.journal_entry.reverse_journal_entry(frm);
			}, __('Make'));
		}

		if (frm.doc.__islocal) {
			frm.add_custom_button(__('Quick Entry'), function() {
				return erpbee.journal_entry.quick_entry(frm);
			});
		}

		// hide /unhide fields based on currency
		erpbee.journal_entry.toggle_fields_based_on_currency(frm);
	...
	make_inter_company_journal_entry: function(frm) {
		...
		d.set_primary_action(__('Create'), function() {
			...
			frappe.call({
				...
				method: "erpbee.accounts.doctype.journal_entry.journal_entry.make_inter_company_journal_entry",
				callback: function (r) {
					if (r.message) {
	...
	posting_date: function(frm) {
		...
		$.each(frm.doc.accounts || [], function(i, row) {
			erpbee.journal_entry.set_exchange_rate(frm, row.doctype, row.name);
		})
	},
	...
	voucher_type: function(frm){
		...
		if((!(frm.doc.accounts || []).length) || ((frm.doc.accounts || []).length === 1 && !frm.doc.accounts[0].account)) {
			if(in_list(["Bank Entry", "Cash Entry"], frm.doc.voucher_type)) {
				return frappe.call({
					type: "GET",
					method: "erpbee.accounts.doctype.journal_entry.journal_entry.get_default_bank_cash_account",
					args: {
				...
				return frappe.call({
					type:"GET",
					method: "erpbee.accounts.doctype.journal_entry.journal_entry.get_opening_accounts",
					args: {
						"company": frm.doc.company
					},
					callback: function(r) {
...
erpbee.accounts.JournalEntry = frappe.ui.form.Controller.extend({
	onload: function() {
		this.load_defaults();
		this.setup_queries();
	...
	setup_queries: function() {
		...
		me.frm.set_query("account", "accounts", function(doc, cdt, cdn) {
			return erpbee.journal_entry.account_query(me.frm);
		});
		...
		me.frm.set_query("party_type", "accounts", function(doc, cdt, cdn) {
			...
			return {
				query: "erpbee.setup.doctype.party_type.party_type.get_party_type",
				filters: {
					'account': row.account
				}
			}
		...
		me.frm.set_query("reference_name", "accounts", function(doc, cdt, cdn) {
			...
			if(jvd.reference_type==="Journal Entry") {
				...
				return {
					query: "erpbee.accounts.doctype.journal_entry.journal_entry.get_against_jv",
					filters: {
						account: jvd.account,
						party: jvd.party
					}
			...
			if(jvd.reference_type==="Payroll Entry") {
				return {
					query: "erpbee.payroll.doctype.payroll_entry.payroll_entry.get_payroll_entries_for_jv",
				};
			}
	...
	get_outstanding: function(doctype, docname, company, child, due_date) {
		...
		return frappe.call({
			method: "erpbee.accounts.doctype.journal_entry.journal_entry.get_outstanding",
			args: { args: args},
			callback: function(r) {
				if(r.message) {
...
cur_frm.script_manager.make(erpbee.accounts.JournalEntry);
...
frappe.ui.form.on("Journal Entry Account", {
	party: function(frm, cdt, cdn) {
		...
		if(!d.account && d.party_type && d.party) {
			...
			return frm.call({
				method: "erpbee.accounts.doctype.journal_entry.journal_entry.get_party_account_and_balance",
				child: d,
				args: {
					company: frm.doc.company,
	...
	cost_center: function(frm, dt, dn) {
		erpbee.journal_entry.set_account_balance(frm, dt, dn);
	},
	...
	debit_in_account_currency: function(frm, cdt, cdn) {
		erpbee.journal_entry.set_exchange_rate(frm, cdt, cdn);
	},
	...
	exchange_rate: function(frm, cdt, cdn) {
		...
		erpbee.journal_entry.set_debit_credit_in_company_currency(frm, cdt, cdn);
	}
})
...
$.extend(erpbee.journal_entry, {
	...
	set_exchange_rate: function(frm, cdt, cdn) {
		...
		if(row.account_currency == company_currency || !frm.doc.multi_currency) {
			...
			frappe.call({
				method: "erpbee.accounts.doctype.journal_entry.journal_entry.get_exchange_rate",
				args: {
		...
		var dialog = new frappe.ui.Dialog({
			...
			fields: [
				...
				{fieldtype: "Link", fieldname: "debit_account", label: __("Debit Account"), reqd: 1,
					options: "Account",
					get_query: function() {
						return erpbee.journal_entry.account_query(frm);
					}
				},
	...
	reverse_journal_entry: function() {
		frappe.model.open_mapped_doc({
			method: "erpbee.accounts.doctype.journal_entry.journal_entry.make_reverse_journal_entry",
			frm: cur_frm
		})
	},
});
...
$.extend(erpbee.journal_entry, {
	set_account_balance: function(frm, dt, dn) {
		...
		if(d.account) {
			...
			return frappe.call({
				method: "erpbee.accounts.doctype.journal_entry.journal_entry.get_account_balance_and_party_type",
				args: {
					account: d.account,
				...
				callback: function(r) {
					if(r.message) {
						$.extend(d, r.message);
						erpbee.journal_entry.set_debit_credit_in_company_currency(frm, dt, dn);
						refresh_field('accounts');
					}
				}
			});
