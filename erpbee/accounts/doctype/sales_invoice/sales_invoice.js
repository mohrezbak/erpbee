...
{% include 'erpbee/selling/sales_common.js' %};


frappe.provide("erpbee.accounts");
erpbee.accounts.SalesInvoiceController = erpbee.selling.SellingController.extend({
	setup: function(doc) {
	...
	onload: function() {
		...
		erpbee.queries.setup_queries(this.frm, "Warehouse", function() {
			return erpbee.queries.warehouse(me.frm.doc);
		});
		...
		erpbee.queries.setup_warehouse_query(this.frm);
	},
		...
		if(doc.docstatus==1 && !doc.is_return) {
			...
			if(!doc.auto_repeat) {
				cur_frm.add_custom_button(__('Subscription'), function() {
					erpbee.utils.make_subscription(doc.doctype, doc.name)
				}, __('Create'))
			}
		}
	...
	make_maintenance_schedule: function() {
		frappe.model.open_mapped_doc({
			method: "erpbee.accounts.doctype.sales_invoice.sales_invoice.make_maintenance_schedule",
			frm: cur_frm
		})
	},
		...
		this.$sales_order_btn = this.frm.add_custom_button(__('Sales Order'),
			function() {
				erpbee.utils.map_current_doc({
					method: "erpbee.selling.doctype.sales_order.sales_order.make_sales_invoice",
	...
	quotation_btn: function() {
		...
		this.$quotation_btn = this.frm.add_custom_button(__('Quotation'),
			function() {
				erpbee.utils.map_current_doc({
					method: "erpbee.selling.doctype.quotation.quotation.make_sales_invoice",
	...
	delivery_note_btn: function() {
		...
		this.$delivery_note_btn = this.frm.add_custom_button(__('Delivery Note'),
			function() {
				erpbee.utils.map_current_doc({
					method: "erpbee.stock.doctype.delivery_note.delivery_note.make_sales_invoice",
					...
					get_query: function() {
						...
						return {
							query: "erpbee.controllers.queries.get_delivery_notes_to_be_billed",
							filters: filters
						};
					}
				});
	...
	customer: function() {
		...
		erpbee.utils.get_party_details(this.frm,
			"erpbee.accounts.party.get_party_details", {
		...
		if(this.frm.doc.customer) {
			frappe.call({
				"method": "erpbee.accounts.doctype.sales_invoice.sales_invoice.get_loyalty_programs",
				"args": {
	...
	make_inter_company_invoice: function() {
		frappe.model.open_mapped_doc({
			method: "erpbee.accounts.doctype.sales_invoice.sales_invoice.make_inter_company_purchase_invoice",
			frm: me.frm
		});
	},
	...
	make_sales_return: function() {
		frappe.model.open_mapped_doc({
			method: "erpbee.accounts.doctype.sales_invoice.sales_invoice.make_sales_return",
			frm: cur_frm
		})
	},
	...
	asset: function(frm, cdt, cdn) {
		...
		if(row.asset) {
			frappe.call({
				method: erpbee.assets.doctype.asset.depreciation.get_disposal_account_and_cost_center,
				args: {
...
$.extend(cur_frm.cscript, new erpbee.accounts.SalesInvoiceController({frm: cur_frm}));

cur_frm.cscript['Make Delivery Note'] = function() {
	frappe.model.open_mapped_doc({
		method: "erpbee.accounts.doctype.sales_invoice.sales_invoice.make_delivery_note",
		frm: cur_frm
	})
}
...
cur_frm.fields_dict['project'].get_query = function(doc, cdt, cdn) {
	return{
		query: "erpbee.controllers.queries.get_project_name",
...
cur_frm.set_query("income_account", "items", function(doc) {
	return{
		query: "erpbee.controllers.queries.get_income_account",
...
cur_frm.cscript.income_account = function(doc, cdt, cdn) {
	erpbee.utils.copy_value_in_all_rows(doc, cdt, cdn, "items", "income_account");
}

cur_frm.cscript.expense_account = function(doc, cdt, cdn) {
	erpbee.utils.copy_value_in_all_rows(doc, cdt, cdn, "items", "expense_account");
}

cur_frm.cscript.cost_center = function(doc, cdt, cdn) {
	erpbee.utils.copy_value_in_all_rows(doc, cdt, cdn, "items", "cost_center");
}
...
frappe.ui.form.on('Sales Invoice', {
	setup: function(frm){
		...
		frm.fields_dict["timesheets"].grid.get_field("time_sheet").get_query = function(doc, cdt, cdn){
			return{
				query: "erpbee.projects.doctype.timesheet.timesheet.get_timesheet",
		...
		frm.fields_dict['items'].grid.get_field('expense_account').get_query = function(doc) {
			if (erpbee.is_perpetual_inventory_enabled(doc.company)) {
				return {
					filters: {
						'report_type': 'Profit and Loss',
						'company': doc.company,
		...
		frm.set_query('pos_profile', function(doc) {
			...
			return {
				query: 'erpbee.accounts.doctype.pos_profile.pos_profile.pos_profile_query',
				filters: {
					company: doc.company
				}
			};
	...
	company:function(frm){
		...
		{
			frappe.call({
				method:"erpbee.setup.doctype.company.company.get_default_company_address",
	...
	loyalty_points: function(frm) {
		if (frm.redemption_conversion_factor) {
			...
			frappe.call({
				method: "erpbee.accounts.doctype.loyalty_program.loyalty_program.get_redeemption_factor",
				args: {
	...
	get_loyalty_details: function(frm) {
		if (frm.doc.customer && frm.doc.redeem_loyalty_points) {
			frappe.call({
				method: "erpbee.accounts.doctype.loyalty_program.loyalty_program.get_loyalty_program_details",
				args: {
	...
	create_invoice_discounting: function(frm) {
		frappe.model.open_mapped_doc({
			method: "erpbee.accounts.doctype.sales_invoice.sales_invoice.create_invoice_discounting",
			frm: frm
		});
	},
	create_dunning: function(frm) {
		frappe.model.open_mapped_doc({
			method: "erpbee.accounts.doctype.sales_invoice.sales_invoice.create_dunning",
			frm: frm
		});
	}
})
...
frappe.ui.form.on('Sales Invoice Timesheet', {
	time_sheet: function(frm, cdt, cdn){
		...
		if(d.time_sheet) {
			frappe.call({
				method: "erpbee.projects.doctype.timesheet.timesheet.get_timesheet_data",
				args: {
					'name': d.time_sheet,
...
var get_healthcare_services_to_invoice = function(frm) {
	...
	dialog.fields_dict["patient"].df.onchange = () => {
		...
		if(patient && patient!=selected_patient){
			...
			var method = "erpbee.healthcare.utils.get_healthcare_services_to_invoice";
...
var get_drugs_to_invoice = function(frm) {
	...
	dialog.fields_dict["encounter"].df.onchange = () => {
		...
		if(encounter && encounter!=selected_encounter){
			...
			var method = "erpbee.healthcare.utils.get_drugs_to_invoice";
