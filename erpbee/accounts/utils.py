...
# imported to enable erpbee.accounts.utils.get_account_currency
from erpbee.accounts.doctype.account.account import get_account_currency
from frappe.model.meta import get_field_precision

from erpbee.stock.utils import get_stock_value_on
...
def check_if_advance_entry_modified(args):
	...
	if args.voucher_type == "Journal Entry":
		...
		party_account_field = ("paid_from"
			if erpbee.get_party_account_type(args.party_type) == 'Receivable' else "paid_to")
...
def get_outstanding_invoices(party_type, party, account, condition=None, filters=None):
	...
	if account:
		...
		party_account_type = erpbee.get_party_account_type(party_type)
...
def get_children(doctype, parent, company, is_root=False):
	from erpbee.accounts.report.financial_statements import sort_accounts
...
def create_payment_gateway_account(gateway, payment_channel="Email"):
	from erpbee.setup.setup_wizard.operations.company_setup import create_bank_account
...
def get_coa(doctype, parent, is_root, chart=None):
	from erpbee.accounts.doctype.account.chart_of_accounts.chart_of_accounts import build_tree_from_json
		...
		if abs(account_bal - stock_bal) > 0.1:
			...
			frappe.msgprint(
				...
				primary_action={
					...
					'client_action': 'erpbee.route_to_adjustment_jv',
