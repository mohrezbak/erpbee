...
frappe.pages['bank-reconciliation'].on_page_load = function(wrapper) {
	new erpbee.accounts.bankReconciliation(wrapper);
}

erpbee.accounts.bankReconciliation = class BankReconciliation {
	constructor(wrapper) {
	...
	add_actions() {
		...
		me.page.add_menu_item(__("Upload a statement"), function() {
			...
			new erpbee.accounts.bankTransactionUpload(me);
		}, true)

		if (me.plaid_status==="active") {
			me.page.add_menu_item(__("Synchronize this account"), function() {
				me.clear_page_content();
				new erpbee.accounts.bankTransactionSync(me);
			}, true)
		}
	...
	make_reconciliation_tool() {
		...
		frappe.model.with_doctype("Bank Transaction", () => {
			erpbee.accounts.ReconciliationList = new erpbee.accounts.ReconciliationTool({
				parent: me.parent,
...
erpbee.accounts.bankTransactionUpload = class bankTransactionUpload {
	constructor(parent) {
		this.parent = parent;
		this.data = [];
	...
	make() {
		...
		new frappe.ui.FileUploader({
			method: 'erpbee.accounts.doctype.bank_transaction.bank_transaction_upload.upload_bank_statement',
			allow_multiple: 0,
	...
	add_bank_entries() {
		const me = this;
		frappe.xcall('erpbee.accounts.doctype.bank_transaction.bank_transaction_upload.create_bank_entries',
...
erpbee.accounts.bankTransactionSync = class bankTransactionSync {
	constructor(parent) {
		this.parent = parent;
		this.data = [];
	...
	init_config() {
		const me = this;
		frappe.xcall('erpbee.erpbee_integrations.doctype.plaid_settings.plaid_settings.get_plaid_configuration')
			.then(result => {
	...
	sync_transactions() {
		...
		frappe.db.get_value("Bank Account", me.parent.bank_account, "bank", (r) => {
			frappe.xcall('erpbee.erpbee_integrations.doctype.plaid_settings.plaid_settings.sync_transactions', {
				bank: r.bank,
...
erpbee.accounts.ReconciliationTool = class ReconciliationTool extends frappe.views.BaseList {
	constructor(opts) {
		super(opts);
		this.show();
	}
	...
	render() {
		...
		me.data.map((value) => {
			...
			new erpbee.accounts.ReconciliationRow(row, value);
		})
	}
...
erpbee.accounts.ReconciliationRow = class ReconciliationRow {
	...
	show_dialog(data) {
		...
		frappe.xcall('erpbee.accounts.page.bank_reconciliation.bank_reconciliation.get_linked_payments',
	...
	make_dialog(data) {
		...
		const fields = [
			...
			},
			{
				...
				get_query: () => {
					...
					if (dt === "Payment Entry") {
						return {
							query: "erpbee.accounts.page.bank_reconciliation.bank_reconciliation.payment_entry_query",
							filters : {
						...
						return {
							query: "erpbee.accounts.page.bank_reconciliation.bank_reconciliation.journal_entry_query",
							filters : {
								"bank_account": this.data.bank_account,
								"company": this.data.company
							}
						...
						return {
							query: "erpbee.accounts.page.bank_reconciliation.bank_reconciliation.sales_invoices_query"
						}
		...
		if (data && data.length > 0) {
			...
			const empty_data_msg = __("ERPBee could not find any matching payment entry")
		...
		$(me.dialog.body).on('click', '.reconciliation-btn', (e) => {
			...
			frappe.xcall('erpbee.accounts.page.bank_reconciliation.bank_reconciliation.reconcile',
				{bank_transaction: me.bank_entry, payment_doctype: payment_doctype, payment_name: payment_entry})
			.then((result) => {
				setTimeout(function(){
					erpbee.accounts.ReconciliationList.refresh();
				}, 2000);
				me.dialog.hide();
			})
		})
