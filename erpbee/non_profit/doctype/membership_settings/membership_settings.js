...
frappe.ui.form.on("Membership Settings", {
	refresh: function(frm) {
		...
		let docs_url = "https://docs.erpbee.com/docs/user/manual/en/non_profit/membership";

		frm.set_intro(__("You can learn more about memberships in the manual. ") + `<a href='${docs_url}'>${__('ERPBee Docs')}</a>`, true);
	...
	add_copy_buttonn: function(frm) {
		if (frm.doc.webhook_secret) {
			frm.add_custom_button(__("Copy Webhook URL"), () => {
				frappe.utils.copy_to_clipboard(`https://${frappe.boot.sitename}/api/method/erpbee.non_profit.doctype.membership.membership.trigger_razorpay_subscription`);
			});
		}
	}
});
