...
frappe.ui.form.on("Stock Reconciliation", {
	...
	get_items: function(frm) {
		frappe.prompt({label:"Warehouse", fieldname: "warehouse", fieldtype:"Link", options:"Warehouse", reqd: 1,
			...
			function(data) {
				frappe.call({
					method:"erpbee.stock.doctype.stock_reconciliation.stock_reconciliation.get_items",
					args: {
	...
	set_valuation_rate_and_qty: function(frm, cdt, cdn) {
		...
		if(d.item_code && d.warehouse) {
			frappe.call({
				method: "erpbee.stock.doctype.stock_reconciliation.stock_reconciliation.get_stock_balance_for",
				args: {
	...
	toggle_display_account_head: function(frm) {
		frm.toggle_display(['expense_account', 'cost_center'],
			erpbee.is_perpetual_inventory_enabled(frm.doc.company));
	},
	purpose: function(frm) {
		frm.trigger("set_expense_account");
	},
	set_expense_account: function(frm) {
		if (frm.doc.company && erpbee.is_perpetual_inventory_enabled(frm.doc.company)) {
			return frm.call({
				method: "erpbee.stock.doctype.stock_reconciliation.stock_reconciliation.get_difference_account",
				args: {
...
erpbee.stock.StockReconciliation = erpbee.stock.StockController.extend({
	setup: function() {
		var me = this;
	...
	refresh: function() {
		if(this.frm.doc.docstatus > 0) {
			...
			if (erpbee.is_perpetual_inventory_enabled(this.frm.doc.company)) {
...
cur_frm.cscript = new erpbee.stock.StockReconciliation({frm: cur_frm});
