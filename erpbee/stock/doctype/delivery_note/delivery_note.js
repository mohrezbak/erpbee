...
frappe.provide("erpbee.stock.delivery_note");
...
frappe.ui.form.on("Delivery Note", {
	setup: function(frm) {
		...
		erpbee.queries.setup_warehouse_query(frm);
	...
	print_without_amount: function(frm) {
		erpbee.stock.delivery_note.set_print_hide(frm.doc);
	},
	...
	refresh: function(frm) {
		...
		if (frm.doc.docstatus === 1 && frm.doc.is_internal_customer && !frm.doc.inter_company_reference) {
			frm.add_custom_button(__('Purchase Receipt'), function() {
				frappe.model.open_mapped_doc({
					method: 'erpbee.stock.doctype.delivery_note.delivery_note.make_inter_company_purchase_receipt',
					frm: frm,
				})
			}, __('Create'));
		}
...
erpbee.stock.DeliveryNoteController = erpbee.selling.SellingController.extend({
	setup: function(doc) {
	...
	refresh: function(doc, dt, dn) {
				...
				this.frm.add_custom_button(__('Sales Order'),
					function() {
						...
						erpbee.utils.map_current_doc({
							method: "erpbee.selling.doctype.sales_order.sales_order.make_delivery_note",
		...
		if (!doc.is_return && doc.status!="Closed") {
			...
			if(doc.docstatus==0 && !doc.__islocal) {
				this.frm.add_custom_button(__('Packing Slip'), function() {
					frappe.model.open_mapped_doc({
						method: "erpbee.stock.doctype.delivery_note.delivery_note.make_packing_slip",
						frm: me.frm
					}) }, __('Create'));
			}
		...
		erpbee.stock.delivery_note.set_print_hide(doc, dt, dn);
	...
	make_shipment: function() {
		frappe.model.open_mapped_doc({
			method: "erpbee.stock.doctype.delivery_note.delivery_note.make_shipment",
			frm: this.frm
		})
	},
	...
	make_sales_return: function() {
		frappe.model.open_mapped_doc({
			method: "erpbee.stock.doctype.delivery_note.delivery_note.make_sales_return",
			frm: this.frm
		})
	},
	...
	make_delivery_trip: function() {
		frappe.model.open_mapped_doc({
			method: "erpbee.stock.doctype.delivery_note.delivery_note.make_delivery_trip",
			frm: cur_frm
		})
	},
	...
	update_status: function(status) {
		...
		frappe.call({
			method:"erpbee.stock.doctype.delivery_note.delivery_note.update_delivery_note_status",
...
$.extend(cur_frm.cscript, new erpbee.stock.DeliveryNoteController({frm: cur_frm}));
...
frappe.ui.form.on('Delivery Note', {
	...
	unhide_account_head: function(frm) {
		...
		var aii_enabled = erpbee.is_perpetual_inventory_enabled(frm.doc.company)
		frm.fields_dict["items"].grid.set_column_disp(["expense_account", "cost_center"], aii_enabled);
	}
})


erpbee.stock.delivery_note.set_print_hide = function(doc, cdt, cdn){
