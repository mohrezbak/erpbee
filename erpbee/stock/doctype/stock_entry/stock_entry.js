...
frappe.ui.form.on('Stock Entry', {
	setup: function(frm) {
		...
		frm.set_query('batch_no', 'items', function(doc, cdt, cdn) {
			...
			if(!item.item_code) {
				...
				return {
					query : "erpbee.controllers.queries.get_batch_no",
					filters: filters
				}
			}
		});
		...
		frm.set_query("expense_account", "additional_costs", function() {
			return {
				query: "erpbee.controllers.queries.tax_account_query",
				filters: {
	...
	setup_quality_inspection: function(frm) {
		...
		frm.set_query("quality_inspection", "items", function(doc, cdt, cdn) {
			...
			return {
				query:"erpbee.stock.doctype.quality_inspection.quality_inspection.quality_inspection_query",
				filters: {
	...
	refresh: function(frm) {
		...
		if(frm.doc.items) {
			...
			if (frm.doc.docstatus == 0 && has_alternative) {
				frm.add_custom_button(__('Alternate Item'), () => {
					erpbee.utils.select_alternate_items({
						frm: frm,
						child_docname: "items",
			...
			if (frm.doc.add_to_transit && frm.doc.purpose=='Material Transfer' && frm.doc.per_transferred < 100) {
				frm.add_custom_button('End Transit', function() {
					frappe.model.open_mapped_doc({
						method: "erpbee.stock.doctype.stock_entry.stock_entry.make_stock_in_entry",
						frm: frm
					})
				});
			}
		...
		if (frm.doc.docstatus===0) {
			frm.add_custom_button(__('Purchase Invoice'), function() {
				erpbee.utils.map_current_doc({
					method: "erpbee.accounts.doctype.purchase_invoice.purchase_invoice.make_stock_entry",
			...
			frm.add_custom_button(__('Material Request'), function() {
				erpbee.utils.map_current_doc({
					method: "erpbee.stock.doctype.material_request.material_request.make_stock_entry",
		...
		if (frm.doc.docstatus===0 && frm.doc.purpose == "Material Issue") {
			frm.add_custom_button(__('Expired Batches'), function() {
				frappe.call({
					method: "erpbee.stock.doctype.stock_entry.stock_entry.get_expired_batch_items",
					callback: function(r) {
						if (!r.exc && r.message) {
							frm.set_value("items", []);
	...
	validate_purpose_consumption: function(frm) {
		frappe.call({
			method: "erpbee.manufacturing.doctype.manufacturing_settings.manufacturing_settings.is_material_consumption_enabled",
		}).then(r => {
			if (cint(r.message) == 0
	...
	set_serial_no: function(frm, cdt, cdn, callback) {
		...
		frappe.call({
			method: "erpbee.stock.get_item_details.get_serial_no",
			args: {"args": args},
			callback: function(r) {
				if (!r.exe && r.message){
	...
	make_retention_stock_entry: function(frm) {
		frappe.call({
			method: "erpbee.stock.doctype.stock_entry.stock_entry.move_sample_to_retention_warehouse",
			args:{
				"company": frm.doc.company,
				"items": frm.doc.items
			},
	...
	toggle_display_account_head: function(frm) {
		var enabled = erpbee.is_perpetual_inventory_enabled(frm.doc.company);
	...
	set_basic_rate: function(frm, cdt, cdn) {
		...
		if (item.item_code || item.serial_no) {
			frappe.call({
				method: "erpbee.stock.utils.get_incoming_rate",
				args: {
					args: args
				},
				callback: function(r) {
	...
	get_warehouse_details: function(frm, cdt, cdn) {
		...
		if(!child.bom_no) {
			frappe.call({
				method: "erpbee.stock.doctype.stock_entry.stock_entry.get_warehouse_details",
				args: {
					"args": {
	...
	get_items_from_bom: function(frm) {
		...
		d.get_input("fetch").on("click", function() {
			...
			frappe.call({
				method: "erpbee.manufacturing.doctype.bom.bom.get_bom_items",
				args: values,
				callback: function(r) {
					if (!r.message) {
						frappe.throw(__("BOM does not contain any stock item"));
					} else {
						erpbee.utils.remove_empty_first_row(frm, "items");
	...
	source_warehouse_address: function(frm) {
		erpbee.utils.get_address_display(frm, 'source_warehouse_address', 'source_address_display', false);
	},

	target_warehouse_address: function(frm) {
		erpbee.utils.get_address_display(frm, 'target_warehouse_address', 'target_address_display', false);
	},
...
frappe.ui.form.on('Stock Entry Detail', {
	...
	barcode: function(doc, cdt, cdn) {
		...
		if (d.barcode) {
			frappe.call({
				method: "erpbee.stock.get_item_details.get_item_code",
	...
	uom: function(doc, cdt, cdn) {
		...
		if(d.uom && d.item_code){
			return frappe.call({
				method: "erpbee.stock.doctype.stock_entry.stock_entry.get_uom_details",
				args: {
					item_code: d.item_code,
					uom: d.uom,
					qty: d.qty
			...
			return frappe.call({
				...
				callback: function(r) {
					if(r.message) {
						...
						if (!d.serial_no) {
							erpbee.stock.select_batch_and_serial_no(frm, d);
						}
					}
				}
			});
	...
	expense_account: function(frm, cdt, cdn) {
		erpbee.utils.copy_value_in_all_rows(frm.doc, cdt, cdn, "items", "expense_account");
	},
	cost_center: function(frm, cdt, cdn) {
		erpbee.utils.copy_value_in_all_rows(frm.doc, cdt, cdn, "items", "cost_center");
	},
...
var validate_sample_quantity = function(frm, cdt, cdn) {
	...
	if (d.sample_quantity && frm.doc.purpose == "Material Receipt") {
		frappe.call({
			method: 'erpbee.stock.doctype.stock_entry.stock_entry.validate_sample_quantity',
			args: {
				batch_no: d.batch_no,
				item_code: d.item_code,
...
erpbee.stock.StockEntry = erpbee.stock.StockController.extend({
	setup: function() {
		var me = this;
		...
		this.frm.fields_dict.items.grid.get_field('item_code').get_query = function() {
			return erpbee.queries.item({is_stock_item: 1});
		};
		...
		if(me.frm.doc.company && erpbee.is_perpetual_inventory_enabled(me.frm.doc.company)) {
			this.frm.add_fetch("company", "stock_adjustment_account", "expense_account");
		}

		this.frm.fields_dict.items.grid.get_field('expense_account').get_query = function() {
			if (erpbee.is_perpetual_inventory_enabled(me.frm.doc.company)) {
				return {
					filters: {
						"company": me.frm.doc.company,
						"is_group": 0
		...
		this.frm.set_query("supplier_address", erpbee.queries.address_query)
	},
	...
	refresh: function() {
		var me = this;
		erpbee.toggle_naming_series();
		this.toggle_related_fields(this.frm.doc);
		this.toggle_enable_bom();
		this.show_stock_ledger();
		if (this.frm.doc.docstatus===1 && erpbee.is_perpetual_inventory_enabled(this.frm.doc.company)) {
			this.show_general_ledger();
		}
		erpbee.hide_company();
		erpbee.utils.add_item(this.frm);
		this.frm.trigger('add_to_transit');
	},

	scan_barcode: function() {
		let transaction_controller= new erpbee.TransactionController({frm:this.frm});
	...
	set_default_account: function(callback) {
		...
		if(this.frm.doc.company && erpbee.is_perpetual_inventory_enabled(this.frm.doc.company)) {
			return this.frm.call({
				method: "erpbee.accounts.utils.get_company_default",
				args: {
	...
	work_order: function() {
		...
		return frappe.call({
			method: "erpbee.stock.doctype.stock_entry.stock_entry.get_work_order_details",
			args: {
	...
	set_warehouse_in_children: function(child_table, warehouse_field, warehouse) {
		let transaction_controller = new erpbee.TransactionController();
		transaction_controller.autofill_warehouse(child_table, warehouse_field, warehouse);
	},

	items_on_form_rendered: function(doc, grid_row) {
		erpbee.setup_serial_no();
	},
	...
	supplier: function(doc) {
		erpbee.utils.get_party_details(this.frm, null, null, null);
	}
});

erpbee.stock.select_batch_and_serial_no = (frm, item) => {
	...
	frappe.require("assets/erpbee/js/utils/serial_no_batch_selector.js", function() {
		new erpbee.SerialNoBatchSelector({
			frm: frm,
			item: item,
...
$.extend(cur_frm.cscript, new erpbee.stock.StockEntry({frm: cur_frm}));
