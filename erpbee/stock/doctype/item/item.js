...
frappe.ui.form.on("Item", {
	...
	refresh: function(frm) {
		...
		if (!frm.doc.is_fixed_asset) {
			erpbee.item.make_dashboard(frm);
		}
		...
		if (frm.doc.has_variants) {
			...
			if(frm.doc.variant_based_on==="Item Attribute") {
				frm.add_custom_button(__("Single Variant"), function() {
					erpbee.item.show_single_variant_dialog(frm);
				}, __('Create'));
				frm.add_custom_button(__("Multiple Variants"), function() {
					erpbee.item.show_multiple_variants_dialog(frm);
				}, __('Create'));
			} else {
				frm.add_custom_button(__("Variant"), function() {
					erpbee.item.show_modal_for_manufacturers(frm);
				}, __('Create'));
			}
		...
		erpbee.item.edit_prices_button(frm);
		erpbee.item.toggle_attributes(frm);
	...
	validate: function(frm){
		erpbee.item.weight_to_validate(frm);
	},
...
$.extend(erpbee.item, {
	setup_queries: function(frm) {
		...
		frm.fields_dict.customer_items.grid.get_field("customer_name").get_query = function(doc, cdt, cdn) {
			return { query: "erpbee.controllers.queries.customer_query" }
		}
	...
	make_dashboard: function(frm) {
		...
		if (frm.doc.is_stock_item) {
			frappe.require('assets/js/item-dashboard.min.js', function() {
				...
				erpbee.item.item_dashboard = new erpbee.stock.ItemDashboard({
					parent: section,
					item_code: frm.doc.name
				});
				erpbee.item.item_dashboard.refresh();
			});
		}
	},
	...
	show_modal_for_manufacturers: function(frm) {
		...
		dialog.set_primary_action(__('Create'), function() {
			...
			frappe.call({
				method: "erpbee.controllers.item_variant.get_variant",
				args: data,
				callback: function(r) {
	...
	show_multiple_variants_dialog: function(frm) {
		...
		function make_and_show_dialog(fields) {
			...
			me.multiple_variant_dialog.set_primary_action(__('Create Variants'), () => {
				...
				frappe.call({
					method: "erpbee.controllers.item_variant.enqueue_multiple_variant_creation",
					args: {
						"item": frm.doc.name,
			...
			frappe.call({
				...
				callback: function(r) {
					...
					if (r.message) {
						...
						d.hide();
						frappe.call({
							method: "erpbee.controllers.item_variant.create_variant",
							args: {
								"item": frm.doc.name,
								"args": d.get_values()
							},
		...
		$.each(d.fields_dict, function(i, field) {
			...
			field.$input
				.on('input', function(e) {
					...
					frappe.call({
						method: "erpbee.stock.doctype.item.item.get_item_attribute",
						args: {
							parent: i,
...
frappe.ui.form.on("UOM Conversion Detail", {
	uom: function(frm, cdt, cdn) {
		...
		if (row.uom) {
			frappe.call({
				method: "erpbee.stock.doctype.item.item.get_uom_conv_factor",
				args: {
					"uom": row.uom,
