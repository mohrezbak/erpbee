...
import frappe
import copy
from erpbee.controllers.item_variant import (ItemVariantExistsError,
		copy_attributes_to_variant, get_variant, make_variant_item_code, validate_item_variant_attributes)
from erpbee.setup.doctype.item_group.item_group import (get_parent_item_groups, invalidate_cache_for)
...
class Item(WebsiteGenerator):
	...
	def set_asset_naming_series(self):
		if not hasattr(self, '_asset_naming_series'):
			from erpbee.assets.doctype.asset.asset import get_asset_naming_series
	...
	def set_metatags(self, context):
		...
		context.metatags['og:site_name'] = 'ERPBee'

	def set_shopping_cart_data(self, context):
		from erpbee.shopping_cart.product_info import get_product_info_for_website
	...
	def recalculate_bin_qty(self, new_name):
		from erpbee.stock.stock_balance import repost_stock
		...
		if self.has_variants:
			...
			if variants:
				if len(variants) <= 30:
					...
					frappe.enqueue("erpbee.stock.doctype.item.item.update_variants",
...
def invalidate_item_variants_cache_for_website(doc):
	from erpbee.portal.product_configurator.item_variants_cache import ItemVariantsCacheManager
