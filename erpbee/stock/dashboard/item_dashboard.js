...
erpbee.stock.ItemDashboard = Class.extend({
	init: function(opts) {
		$.extend(this, opts);
		this.make();
	},
		...
		function handle_move_add(element, action) {
			...
			if (disable_quick_entry) {
				...
				if (action === "Add") {
					...
					erpbee.stock.move_item(item, null, warehouse, actual_qty, rate, function() { me.refresh(); });
				}
				else {
					erpbee.stock.move_item(item, warehouse, null, actual_qty, null, function() { me.refresh(); });
				}
			}
		}
	...
	refresh: function() {
		...
		var me = this;
		frappe.call({
			method: 'erpbee.stock.dashboard.item_dashboard.get_data',
			args: {
...
erpbee.stock.move_item = function(item, source, target, actual_qty, rate, callback) {
	...
	dialog.set_primary_action(__('Submit'), function() {
		...
		frappe.call({
			method: 'erpbee.stock.doctype.stock_entry.stock_entry_utils.make_stock_entry',
			args: values,
			freeze: true,
			callback: function(r) {
