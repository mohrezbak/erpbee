...
import json, copy
from erpbee.accounts.doctype.pricing_rule.pricing_rule import get_pricing_rule_for_item, set_transaction_type
from erpbee.setup.utils import get_exchange_rate
from frappe.model.meta import get_field_precision
from erpbee.stock.doctype.batch.batch import get_batch_no
from erpbee import get_company_currency
from erpbee.stock.doctype.item.item import get_item_defaults, get_uom_conv_factor
from erpbee.stock.doctype.price_list.price_list import get_price_list_details
from erpbee.setup.doctype.item_group.item_group import get_item_group_defaults
from erpbee.setup.doctype.brand.brand import get_brand_defaults
from erpbee.stock.doctype.item_manufacturer.item_manufacturer import get_item_manufacturer_part_no
...
def validate_item_details(args, item):
	...
	from erpbee.stock.doctype.item.item import validate_end_of_life
...
def get_basic_details(args, item, overwrite_warehouse=True):
	...
	if args.get('doctype') in purchase_doctypes:
		from erpbee.buying.doctype.purchase_order.purchase_order import item_last_purchase_rate
...
def get_default_cost_center(args, item=None, item_group=None, brand=None, company=None):
	...
	if not cost_center and (item and item_group and brand):
		...
		for method in ["get_item_defaults", "get_item_group_defaults", "get_brand_defaults"]:
			path = "erpbee.stock.get_item_details.{0}".format(method)
...
def get_price_list_rate(args, item_doc, out):
	...
	if meta.get_field("currency") or args.get('currency'):
		...
		if not out.price_list_rate and args.transaction_type=="buying":
			from erpbee.stock.doctype.item.item import get_last_purchase_details
...
def validate_conversion_rate(args, meta):
	from erpbee.controllers.accounts_controller import validate_conversion_rate
...
def get_batch_qty(batch_no, warehouse, item_code):
	from erpbee.stock.doctype.batch import batch
	if batch_no:
