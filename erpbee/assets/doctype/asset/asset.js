...
frappe.ui.form.on('Asset', {
	...
	setup: function(frm) {
		frm.make_methods = {
			'Asset Movement': () => {
				frappe.call({
				method: "erpbee.assets.doctype.asset.asset.make_asset_movement",
				freeze: true,
				args:{
		...
		frm.set_query("purchase_receipt", (doc) => {
			return {
				query: "erpbee.controllers.queries.get_purchase_receipts",
				filters: { item_code: doc.item_code }
			}
		});
		frm.set_query("purchase_invoice", (doc) => {
			return {
				query: "erpbee.controllers.queries.get_purchase_invoices",
				filters: { item_code: doc.item_code }
			}
		});
	},
		...
		if (frm.doc.docstatus==1) {
			if (in_list(["Submitted", "Partially Depreciated", "Fully Depreciated"], frm.doc.status)) {
				frm.add_custom_button("Transfer Asset", function() {
					erpbee.asset.transfer_asset(frm);
				});

				frm.add_custom_button("Scrap Asset", function() {
					erpbee.asset.scrap_asset(frm);
				});
				...
				frm.add_custom_button("Restore Asset", function() {
					erpbee.asset.restore_asset(frm);
				});
			}
	...
	make_journal_entry: function(frm) {
		frappe.call({
			method: "erpbee.assets.doctype.asset.asset.make_journal_entry",
			args: {
	...
	set_finance_book: function(frm) {
		frappe.call({
			method: "erpbee.assets.doctype.asset.asset.get_item_details",
			args: {
	...
	opening_accumulated_depreciation: function(frm) {
		erpbee.asset.set_accumulated_depreciation(frm);
	},
	...
	make_sales_invoice: function(frm) {
		frappe.call({
			...
			method: "erpbee.assets.doctype.asset.asset.make_sales_invoice",
			callback: function(r) {
				var doclist = frappe.model.sync(r.message);
				frappe.set_route("Form", doclist[0].doctype, doclist[0].name);
			}
	...
	create_asset_maintenance: function(frm) {
		frappe.call({
			...
			method: "erpbee.assets.doctype.asset.asset.create_asset_maintenance",
			callback: function(r) {
				var doclist = frappe.model.sync(r.message);
				frappe.set_route("Form", doclist[0].doctype, doclist[0].name);
			}
	...
	create_asset_adjustment: function(frm) {
		frappe.call({
			...
			method: "erpbee.assets.doctype.asset.asset.create_asset_adjustment",
			freeze: 1,
			callback: function(r) {
				var doclist = frappe.model.sync(r.message);
				frappe.set_route("Form", doclist[0].doctype, doclist[0].name);
...
frappe.ui.form.on('Depreciation Schedule', {
	make_depreciation_entry: function(frm, cdt, cdn) {
		...
		if (!row.journal_entry) {
			frappe.call({
				method: "erpbee.assets.doctype.asset.depreciation.make_depreciation_entry",
				args: {
					"asset_name": frm.doc.name,
...
erpbee.asset.set_accumulated_depreciation = function(frm) {
...
erpbee.asset.scrap_asset = function(frm) {
	frappe.confirm(__("Do you really want to scrap this asset?"), function () {
		frappe.call({
			args: {
				"asset_name": frm.doc.name
			},
			method: "erpbee.assets.doctype.asset.depreciation.scrap_asset",
			callback: function(r) {
				cur_frm.reload_doc();
			}
		})
...
erpbee.asset.restore_asset = function(frm) {
	frappe.confirm(__("Do you really want to restore this scrapped asset?"), function () {
		frappe.call({
			args: {
				"asset_name": frm.doc.name
			},
			method: "erpbee.assets.doctype.asset.depreciation.restore_asset",
			callback: function(r) {
				cur_frm.reload_doc();
			}
		})
...
erpbee.asset.transfer_asset = function() {
	frappe.call({
		method: "erpbee.assets.doctype.asset.asset.make_asset_movement",
		freeze: true,
		args:{
