...
from erpbee.accounts.party import validate_party_accounts, get_dashboard_info, get_timeline_data # keep this
...
def get_customer_list(doctype, txt, searchfield, start, page_len, filters=None):
	from erpbee.controllers.queries import get_fields
	...
	if credit_limit > 0 and flt(customer_outstanding) > credit_limit:
		...
		if not credit_controller_role or credit_controller_role not in frappe.get_roles():
			...
			frappe.msgprint(message,
				...
				primary_action={
					'label': 'Send Email',
					'server_action': 'erpbee.selling.doctype.customer.customer.send_emails',
					'args': {
						'customer': customer,
